...
class KeyboardState(private val switchActions: SwitchActions) {
    ...
    private fun toggleFloatingMode() {
        if (DebugFlags.DEBUG_ENABLED) {
            Log.d(TAG, "toggleFloatingMode")
        }
        switchActions.toggleFloatingMode()
    }

    fun onCodeInput(code: Int, autoCapsFlags: Int, recapitalizeMode: RecapitalizeMode?) {
        if (DEBUG_EVENT) {
            Log.d(TAG, ("onCodeInput: code=${Constants.printableCode(code)} ${stateToString(autoCapsFlags, recapitalizeMode)}"))
        }
        ...
        if (Constants.isLetterCode(code)) {
            ...
            KeyCode.TOGGLE_FLOATING_MODE -> toggleFloatingMode()
        }
    }
