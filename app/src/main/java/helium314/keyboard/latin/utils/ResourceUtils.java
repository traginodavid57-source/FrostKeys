...
public final class ResourceUtils {
    ...
    public static int getKeyboardWidth(final Context ctx, final SettingsValues settingsValues) {
        ...
        if (settingsValues != null && settingsValues.mIsFloatingModeEnabled) {
            final boolean isLandscape = ctx.getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE;
            final float baseScale = isLandscape ? 0.55f : 0.75f;
            final float effectiveScale = Math.max(0.40f, Math.min(1.0f, baseScale * settingsValues.mFloatingKeyboardScale));
            return Math.round(effectiveScale * defaultKeyboardWidth);
        }
        if (settingsValues != null && settingsValues.mOneHandedModeEnabled) {
    ...
    public static int getKeyboardHeight(final Resources res, final SettingsValues settingsValues) {
        ...
        if (settingsValues.mIsFloatingModeEnabled) {
            final boolean isLandscape = res.getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE;
            final float baseScale = isLandscape ? 0.65f : 0.85f;
            scale *= Math.max(0.45f, Math.min(1.0f, baseScale * settingsValues.mFloatingKeyboardScale));
        }
